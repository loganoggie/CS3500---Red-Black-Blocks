...
void placeRedBlockUnit(int minRedBlockUnitSize, int redBlockUnitSize, int rowSize, int start, vector<string>& configurations, std::string prevString);
...
void placeRedBlockUnit(int minRedBlockUnitSize, int redBlockUnitSize, int rowSize, int start, vector<string>& configurations, std::string prevString)
{
    if (rowSize >= (redBlockUnitSize + start))
    {
        ...
        placeRedBlockUnit(minRedBlockUnitSize, redBlockUnitSize, rowSize, start+redBlockUnitSize+1, configurations, newConfig);
        // additionally, try R block units of other sizes
        for (int otherRedBlockUnitSize = minRedBlockUnitSize; otherRedBlockUnitSize <= rowSize; otherRedBlockUnitSize++)
            if (otherRedBlockUnitSize != redBlockUnitSize)
                placeRedBlockUnit(minRedBlockUnitSize, otherRedBlockUnitSize, rowSize, start+redBlockUnitSize+1, configurations, newConfig);
    }
}
...
void placeRedBlocks(int rowSize)
{
    ...
    for (int i = minRedBlockUnitSize; i <= rowSize; i++)
        ...
        {
            placeRedBlockUnit(minRedBlockUnitSize, i, rowSize, start, configurations, allBlack);
        }
    for (int i = 0; i < configurations.size(); i++)
        cout << configurations[i] << endl;
    cout << "\n# configurations for row size " << rowSize << " is " << configurations.size() << endl;
}
